...
# local run
# python -m experiments.dcase22.t1.passt_congater with pretrained_256 trainer.precision=16 basedataset.audio_processor.resample_only=True basedataset.audio_processor.sr=32000 models.net.s_patchout_f=6 trainer.max_epochs=10 warm_up_len=30 ramp_down_start=30 ramp_down_len=100  basedataset.server=0 wandb=0

@dataset.config
def default_config():
    server = 1
    name = 'd22t1'  # dataset name # resampledataset: 'd22t1_r'
    # processed audio content files cached in this location
    cache_root_path = f"{'' if server else '~'}/share/rk6/shared/kofta_cached_datasets/"
    # base directory of the dataset as downloaded and reassembled
    base_dir = f"{'' if server else '~'}/share/rk6/shared/dcase22/" # dcase22_reassembled
    ...
    subsample = 0
    # eval files
    eval_base_dir = f"{'' if server else '~'}/share/rk6/shared/dcase22/"
