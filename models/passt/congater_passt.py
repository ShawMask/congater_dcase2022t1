...
def pretrained_passt(model_number=253, dcase_weights=False):
    path_ = os.path.join('teacher_models', f"passt_{model_number}.pt")
    print(path_)
    model = get_model(arch="passt_s_swa_p16_128_ap476", fstride=10, tstride=10, n_classes=10)
...
class ConGaterPaSST(nn.Module):
    def __init__(self, model_number=253, dcase_weights=False, num_gate_layers=1, domain='device location',
                 gate_activation="t",):
        super(ConGaterPaSST, self).__init__()

        self.passt = pretrained_passt(model_number, dcase_weights=dcase_weights)
        ...
        for i in self.domain:
            ...
            for _ in range(len(self.passt.blocks)):
                if domain=="none":
                    self.congater[i].append(GateLayer(2, num_layers=num_gate_layers,
                                                      gate_activation=gate_activation))
                else:
                    self.congater[i].append(GateLayer(self.passt.embed_dim, num_layers=num_gate_layers,
                                                      gate_activation=gate_activation))
        self.first_run = 1
    ...
    def forward_gate(self, x, temp, layer, active_gates='device location'):
        x_gate = []
        counter = 0
        if 'none' not in self.domain:
            for domain_gate in self.domain:
                    if domain_gate in active_gates:
                        x_gate.append(self.congater[domain_gate][layer](x, temp[counter]))
                        # print(f"{domain_gate}/{self.domain},temp{temp}", x_gate[-1])
                    else:
                        x_gate.append(self.congater[domain_gate][layer](x, 0))
                        # print(f"{domain_gate}/{self.domain},temp{temp}", x_gate[-1])
        else:
    ...
    def forward_features(self, x, temp=[0, 0], active_gates='device location'):
        ...
        if self.first_run: print(" patch_embed : ", x.shape)
        # Adding Time/Freq information
        if self.first_run: print(" self.time_new_pos_embed.shape", self.passt.time_new_pos_embed.shape)
        ...
        if x.shape[-1] < time_new_pos_embed.shape[-1]:
            if self.passt.training:
                ...
                if self.first_run: print(f" CUT with randomoffset={toffset} time_new_pos_embed.shape",
            ...
            if self.first_run: print(" CUT time_new_pos_embed.shape", time_new_pos_embed.shape)
        else:
            warnings.warn(
        ...
        if self.first_run: print(" self.freq_new_pos_embed.shape", self.passt.freq_new_pos_embed.shape)
        ...
        if self.passt.training and self.passt.s_patchout_t:
            if self.first_run: print(f"X Before time Patchout of {self.passt.s_patchout_t} ", x.size())
            # ([1, 768, 1, 82])
            random_indices = torch.randperm(T_dim)[:T_dim - self.passt.s_patchout_t].sort().values
            x = x[:, :, :, random_indices]
            if self.first_run: print("X after time Patchout", x.size())
        if self.passt.training and self.passt.s_patchout_f:
            if self.first_run: print(f"X Before Freq Patchout of {self.passt.s_patchout_f} ", x.size())
            # [1, 768, 12, 1]
            random_indices = torch.randperm(F_dim)[:F_dim - self.passt.s_patchout_f].sort().values
            x = x[:, :, random_indices, :]
            if self.first_run: print(" \n X after freq Patchout: ", x.size())
        ###
        # Flatten the sequence
        x = x.flatten(2).transpose(1, 2)
        # Unstructured Patchout
        if self.first_run: print("X flattened", x.size())
        if self.passt.training and self.passt.u_patchout:
            ...
            x = x[:, random_indices, :]
            if self.first_run: print("X After Unstructured Patchout", x.size())
        ####
        # Add the C/D tokens
        if self.first_run: print(" self.new_pos_embed.shape", self.passt.new_pos_embed.shape)
        cls_tokens = self.passt.cls_token.expand(B_dim, -1, -1) + self.passt.new_pos_embed[:, :1, :]
        if self.first_run: print(" self.cls_tokens.shape", cls_tokens.shape)
        if self.passt.dist_token is None:
            x = torch.cat((cls_tokens, x), dim=1)
        else:
            dist_token = self.passt.dist_token.expand(B_dim, -1, -1) + self.passt.new_pos_embed[:, 1:, :]
            if self.first_run: print(" self.dist_token.shape", dist_token.shape)
            x = torch.cat((cls_tokens, dist_token, x), dim=1)

        if self.first_run: print(" final sequence x", x.shape)
        x = self.passt.pos_drop(x)
        for cnt, block in enumerate(self.passt.blocks):
            x = block(x)*self.forward_gate(x, temp, cnt, active_gates) # Self Gate of the ConGater
        if self.first_run: print(f" after {len(self.passt.blocks)} atten blocks x", x.shape)
        x = self.passt.norm(x)
        if self.passt.dist_token is None:
    ...
    def forward(self, x, temp=[0, 0], active_gates='device location'):
        # global self.first_run
        if self.first_run: print("x", x.size())
        x = self.forward_features(x, temp, active_gates)
        if self.passt.head_dist is not None:
            features = (x[0] + x[1]) / 2
            if self.first_run: print("forward_features", features.size())
            x = self.passt.head(features)
            if self.first_run: print("head", x.size())
...
model_ing = Ingredient("congater_passt")

model_ing.add_config(instance_cmd="load_congater_passt")
# model_ing.add_config(instance_cmd="get_model")


@model_ing.command
def load_congater_passt(model_number=253, dcase_weights=False, num_gate_layers=1, domain='device location',
                        gate_activation="t"):
    return ConGaterPaSST(model_number, dcase_weights, num_gate_layers, domain, gate_activation)
